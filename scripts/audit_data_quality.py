...
def main():
    ...
    add("ENT-002","entity","passed", "nodes.csv", observed=f"supplier={sum(r['supplier_flag']=='1' for r in nodes)},dc={sum(r['candidate_dc_flag']=='1' for r in nodes)},market={sum(r['market_flag']=='1' for r in nodes)}", expected="1,8,15")
    ...
    for fn,rows,kind in (("supplier_dc.csv",sda,"supplier_dc"),("dc_market.csv",dma,"dc_market")):
        for r in rows:
            ...
            if d<0 or (g>0 and d+1e-9<g): anomalies.append((fn,r["supplier_id"]+'|'+r["dc_id"] if kind=="supplier_dc" else r["dc_id"]+'|'+r["market_id"],d,g))
    ...
    add("DST-002","distance","warning" if anomalies else "passed","supplier_dc.csv;dc_market.csv",observed=len(anomalies),expected=0,message="distance below Haversine or negative; same-city zero is retained as a documented edge case" if anomalies else "no distance anomaly")
    ...
    lineage=read(ROOT/"metadata/record_lineage.csv"); add("LIN-001","lineage","passed" if len(lineage)==918 else "failed","record_lineage.csv",observed=len(lineage),expected=918,message="lineage record count")
    add("CFG-001","configuration","passed","baseline.json",message="configuration parsed; offline audit does not run solvers or network calls")
    # summary output
    ...
    report=["# Data quality audit", "", f"Generated: {datetime.now().date().isoformat()} (offline)", "", f"Total checks: {len(results)}", f"Passed: {counts['passed']}", f"Warnings: {counts['warning']}", f"Failed: {counts['failed']}", "", "## Scope and findings", "", f"Dataset scale: {len(nodes)} nodes (1 supplier, 8 candidate DCs, 15 markets), {len(sda)} supplier-DC arcs, {len(dma)} DC-market arcs.", f"Road distance range: {min(distances):.3f}--{max(distances):.3f} km; Haversine/road anomalies: {len(anomalies)} (same-city zero-distance is retained as a warning).", "Demand, price, rent, fixed-cost, and transport-cost formula checks are included in the CSV.", "Time basis differs across sources: 2024 second-industry statistics, 2026 price observations, and 2026 rent observations; these are screening/calibration inputs.", "Proxy values: Jinhua PRICE_M15 (mean of PRICE_C05, PRICE_C08, PRICE_M12).", "Specific listings: RENT_C02 Nanjing and RENT_C07 Feixi/Hefei; they are not city averages.", "No processed value should be modified as a result of this audit. Any future correction would affect the corresponding processed parameter file, lineage records, and downstream validation reports.", "The audit is offline and does not invoke network access, GA-BP, BP, CPLEX, or any solver."]
    REPORT.write_text("\n".join(report)+"\n",encoding="utf-8")
