...
def main():
 ...
 with (ROOT/'metadata/v0.2_intracity_transport_emissions.csv').open('w',encoding='utf-8-sig',newline='') as f:
  w=csv.DictWriter(f,fieldnames=['arc_id','representative_distance_km','transport_emission_factor_tco2e_per_tonne_km','transport_emission_tco2e_per_tonne','source_type']); w.writeheader(); [w.writerow({'arc_id':k,'representative_distance_km':f'{args.distance_km:.3f}','transport_emission_factor_tco2e_per_tonne_km':'0.00006996','transport_emission_tco2e_per_tonne':f'{args.distance_km*0.00006996:.8f}','source_type':'researcher_defined_scenario_parameter'}) for k in sorted(ARC_IDS)]
